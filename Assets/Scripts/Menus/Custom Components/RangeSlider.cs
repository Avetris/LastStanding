...
namespace UnityEngine.UI.Extensions
{
    ...
    public class RangeSlider : Selectable, IDragHandler, IInitializePotentialDragHandler, ICanvasElement
    {
        ...
        [SerializeField]
        public TMP_Text m_OutputFromText;
        [SerializeField]
        public TMP_Text m_OutputToText;
        ...
        public float StepSize { get { return m_StepSize; } set { /*if (WholeNumbers) { m_StepSize = Mathf.Round(m_StepSize); */UpdateVisuals(); /*}*/ } }
        ...
        public virtual float HighValue
        {
            get
            {
                return GetInterval(m_HighValue);
            }
            set
            {
                SetHigh(value);
        ...
        protected override void OnValidate()
        {
            ...
            if (m_StepSize != 0.0f)
            {
                m_MinValue -= m_MinValue % m_StepSize;
                m_MaxValue -= m_MaxValue % m_StepSize;
            }
        ...
        protected virtual void SetLow(float input, bool sendCallback)
        {
            ...
            float newValue = Mathf.Clamp(input, MinValue, HighValue); //clamp between min and High
                                                                      // if (WholeNumbers)
                                                                      // {
                                                                      // m_StepSize = Mathf.Round(m_StepSize);
            newValue = GetInterval(newValue);
            // }
        ...
        {
            bool noDecimals = m_StepSize == Mathf.Round(m_StepSize);
            if (m_OutputFromText != null)
            {
                m_OutputFromText.SetText(m_LowValue.ToString(noDecimals ? "f0" : "f1"));
            }

            if (m_OutputToText != null)
            {
                m_OutputToText.SetText(m_HighValue.ToString(noDecimals ? "f0" : "f1"));
            }
        }

        private float GetInterval(float input)
        {
            if (StepSize == 0.0f) return input;
            float fromInitial = input - m_MinValue;

            float mod = fromInitial % StepSize;
