...
public class EOSLobby : MonoBehaviour
{
    ...
    private List<Attribute> currentLobbyData = new List<Attribute>();
    private bool isLobbyOwner = false;

    public AttributeDataValue GetLobbyData(Enumerators.GameSetting key)
    {
        foreach (Attribute attribute in currentLobbyData)
        {
            if (key.ToString().Equals(attribute.Data.Key))
            {
    ...
    public List<Attribute> GetCurrentData() { return currentLobbyData; }
    ...
    public delegate void FindCodeLobbySucess(LobbyDetails lobbyDetails);
    public event FindCodeLobbySucess FindCodeLobbySucceeded;
    public delegate void FindCodeLobbyFailure(bool notFound, string errorMessage);
    public event FindCodeLobbyFailure FindCodeLobbyFailed;
    ...
    public virtual void CreateLobby(uint maxConnections, LobbyPermissionLevel permissionLevel, bool presenceEnabled, uint maxMembers, AttributeData[] lobbyData = null)
    {
        ...
        }, null, (CreateLobbyCallbackInfo callback) =>
        {
            ...
            modHandle.SetMaxMembers(new LobbyModificationSetMaxMembersOptions { MaxMembers = maxMembers });
            AttributeData defaultData = new AttributeData { Key = DefaultAttributeKey, Value = DefaultAttributeKey };
            AttributeData hostAddressData = new AttributeData { Key = Enumerators.GameSetting.Host_Address.ToString(), Value = EOSSDKComponent.LocalUserProductIdString };
            ...
            if (permissionLevel == LobbyPermissionLevel.Publicadvertised)
            {
                while (lobbyId == null)
                {
                    lobbyId = Utils.GenerateRandomId(Constants.LobbyCodeLength);
                    ...
                    search.SetParameter(new LobbySearchSetParameterOptions
                    {
                        ...
                        Parameter = new AttributeData { Key = Enumerators.GameSetting.Lobby_Id.ToString(), Value = lobbyId }
                    });
                    ...
                    {
                        if (callback.ResultCode != Result.Success || search.GetSearchResultCount(new LobbySearchGetSearchResultCountOptions { }) > 0)
                        {
                            lobbyId = null;
                        }
                    });
            ...
            if (lobbyId != null)
            {
                AttributeData lobbyIdAttribute = new AttributeData { Key = Enumerators.GameSetting.Lobby_Id.ToString(), Value = lobbyId };
                modHandle.AddAttribute(new LobbyModificationAddAttributeOptions { Attribute = lobbyIdAttribute, Visibility = LobbyAttributeVisibility.Public });
                lobbyReturnData.Add(new Attribute { Data = lobbyIdAttribute, Visibility = LobbyAttributeVisibility.Public });
            }
            ...
            EOSSDKComponent.GetLobbyInterface().UpdateLobby(new UpdateLobbyOptions { LobbyModificationHandle = modHandle }, null, (UpdateLobbyCallbackInfo updateCallback) =>
            {
                ...
                Debug.Log($"New Lobby id: {currentLobbyId} -- custom id: {lobbyId}");
                currentLobbyData = lobbyReturnData;
    ...
    public virtual void SearchLobbyById(string lobbyId)
    {
        //create search handle and list of lobby details
        LobbySearch search = new LobbySearch();
        ...
        search.Find(new LobbySearchFindOptions { LocalUserId = EOSSDKComponent.LocalUserProductId }, null, (LobbySearchFindCallbackInfo callback) =>
        {
            ...
            if (callback.ResultCode != Result.Success)
            {
                FindCodeLobbyFailed?.Invoke(false, "There was an error while finding lobby. Error: " + callback.ResultCode);
                return;
            }

            if (search.GetSearchResultCount(new LobbySearchGetSearchResultCountOptions { }) == 0)
            {
                FindCodeLobbyFailed?.Invoke(true, $"Lobby code not found. Code: {lobbyId}");
                return;
            }
            ...
            //invoke event
            FindCodeLobbySucceeded?.Invoke(lobbyInformation);
    ...
    public virtual void JoinLobby(LobbyDetails lobbyToJoin, string[] attributeKeys = null, bool presenceEnabled = false)
    {
        ...
        EOSSDKComponent.GetLobbyInterface().JoinLobby(new JoinLobbyOptions { LobbyDetailsHandle = lobbyToJoin, LocalUserId = EOSSDKComponent.LocalUserProductId, PresenceEnabled = presenceEnabled }, null, (JoinLobbyCallbackInfo callback) =>
        {
            ...
            lobbyToJoin.CopyAttributeByKey(new LobbyDetailsCopyAttributeByKeyOptions { AttrKey = Enumerators.GameSetting.Host_Address.ToString() }, out hostAddress);
